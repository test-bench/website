...
Because TestBench is designed to cooperate with the surrounding operating system, there are many things that can be done with TestBench beyond what it offers out of the box. These recipes are therefore intended to offer TestBench users the benefit of features commonly found in other test frameworks. However, there is an additional benefit to studying them: by seeing how TestBench can be extended with shell scripting and readily available UNIX tools, users can learn to address all sorts of emergent scenarios with a bit of improvising.
